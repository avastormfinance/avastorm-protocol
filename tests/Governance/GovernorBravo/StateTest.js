...
describe('GovernorBravo#state/1', () => {
  let alt, gov, root, acct, delay, timelock;
  ...
  beforeAll(async () => {
    ...
    gov = await deploy('GovernorBravoImmutable', [timelock._address, alt._address, root, 17280, 1, "100000000000000000000000"]);
    await send(gov, '_initiate');
    await send(timelock, "harnessSetAdmin", [gov._address])
    await send(alt, 'transfer', [acct, etherMantissa(4000000)]);
    await send(alt, 'delegate', [acct], { from: acct });
  });
  ...
  it("Canceled", async () => {
    await send(alt, 'transfer', [accounts[0], etherMantissa(4000000)]);
    await send(alt, 'delegate', [accounts[0]], { from: accounts[0] });
    await mineBlock()
    ...
    await send(alt, 'delegate', [root], { from: accounts[0] });
