        ...
        if (eventArg === undefined) {
          ...
          try {
            ...
            if (arg.rescue) {
              ...
              // `Gate (AToken Eth Address) (... deploy aToken)`
              // could be used to deploy a aToken if it doesn't exist, but
              // since there is no AToken, that check would raise (when we'd
