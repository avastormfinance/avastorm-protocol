...
async function setPrice(world: World, from: string, priceOracle: PriceOracle, aToken: string, amount: NumberV): Promise<World> {
  return addAction(
    world,
...
export function priceOracleCommands() {
  return [
    ...
    new Command<{priceOracle: PriceOracle, aToken: AddressV, amount: NumberV}>(`
        #### SetPrice
    ...
    new Command<{priceOracle: PriceOracle, address: AddressV, amount: NumberV}>(`
        ...
        * "SetDirectPrice <Address> <Amount>" - Sets the per-ether price for the given aToken
