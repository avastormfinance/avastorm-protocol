...
import { altCommands, processAltEvent } from './Event/AltEvent';
...
import { AvastormLens } from './Contract/AvastormLens';
...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] = [
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Expect ...expectation" - Adds an expectation to hold after the next transaction
        * E.g. "Expect Changes (AToken cZRX TotalSupply) +10.0e18"
    `,
    'Expect',
    [new Arg('event', getEventV, { variadic: true })],
  ...
  new Command<{ event: EventV }>(
    ...
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
      return processAltEvent(world, event.val, from);
    },
    { subExpressions: altCommands() }
  ),
  ...
  buildContractEvent<AvastormLens>("AvastormLens", false),
