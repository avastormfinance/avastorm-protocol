...
contract ComptrollerG7 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     * @param altSpeed New ALT speed for market
     */
    function setAltSpeedInternal(AToken aToken, uint altSpeed) internal {
        uint currentAltSpeed = altSpeeds[address(aToken)];
        if (currentAltSpeed != 0) {
            // note that ALT speed could be set to 0 to halt liquidity rewards for a market
            ...
            if (altSupplyState[address(aToken)].index == 0 && altSupplyState[address(aToken)].block == 0) {
                altSupplyState[address(aToken)] = AltMarketState({
                    index: altInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }

            if (altBorrowState[address(aToken)].index == 0 && altBorrowState[address(aToken)].block == 0) {
                altBorrowState[address(aToken)] = AltMarketState({
                    index: altInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        ...
        if (currentAltSpeed != altSpeed) {
            altSpeeds[address(aToken)] = altSpeed;
            emit AltSpeedUpdated(aToken, altSpeed);
        }
    }
     ...
     * @param altSpeed New ALT speed for market
     */
    function _setAltSpeed(AToken aToken, uint altSpeed) public {
        require(adminOrInitializing(), "only admin can set alt speed");
        setAltSpeedInternal(aToken, altSpeed);
    }
