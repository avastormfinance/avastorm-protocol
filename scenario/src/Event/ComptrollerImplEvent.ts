...
async function becomeG3(
  ...
  let invokation = await invoke(
    world,
    comptrollerImpl.methods._become(unitroller._address, altRate, altMarkets, otherMarkets),
    from,
    ComptrollerErrorReporter
  );
...
export function comptrollerImplCommands() {
  return [
    ...
    new Command<{
      ...
      comptrollerImpl: ComptrollerImpl;
      altRate: NumberV;
      altMarkets: ArrayV<AddressV>;
      otherMarkets: ArrayV<AddressV>;
    }>(
      `
        #### BecomeG3

        * "ComptrollerImpl <Impl> BecomeG3 <Rate> <AltMarkets> <OtherMarkets>" - Become the comptroller, if possible.
      ...
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
        new Arg('comptrollerImpl', getComptrollerImpl),
        new Arg('altRate', getNumberV, { default: new NumberV(1e18) }),
        new Arg('altMarkets', getArrayV(getAddressV),  {default: new ArrayV([]) }),
        new Arg('otherMarkets', getArrayV(getAddressV), { default: new ArrayV([]) })
      ],
      (world, from, { unitroller, comptrollerImpl, altRate, altMarkets, otherMarkets }) => {
        return becomeG3(world, from, comptrollerImpl, unitroller, altRate.encode(), altMarkets.val.map(a => a.val), otherMarkets.val.map(a => a.val))
      },
      { namePos: 1 }
    ),
    ...
    new Command<{
      ...
      `
        ...
        * "ComptrollerImpl <Impl> Become <Rate> <AltMarkets> <OtherMarkets>" - Become the comptroller, if possible.
