...
describe.skip('Flywheel trace ops', () => {
  ...
  beforeEach(async () => {
    ...
    await send(comptroller, '_addAltMarkets', [[market].map(c => c._address)]);
  });
  ...
  it('update supply index SSTOREs', async () => {
    ...
    await send(comptroller, 'setAltSpeed', [market._address, etherExp(0.5)]);

    const tx = await send(comptroller, 'harnessUpdateAltSupplyIndex', [market._address]);
  ...
  it('update borrow index SSTOREs', async () => {
    ...
    const tx = await send(comptroller, 'harnessUpdateAltBorrowIndex', [market._address, etherExp(1.1)]);
