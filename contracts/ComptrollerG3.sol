...
contract ComptrollerG3 is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function _become(Unitroller unitroller, uint altRate_, address[] memory altMarketsToAdd, address[] memory otherMarketsToAdd) public {
        require(msg.sender == unitroller.admin(), "only unitroller admin can change brains");
        require(unitroller._acceptImplementation() == 0, "change not authorized");

        ComptrollerG3(address(unitroller))._becomeG3(altRate_, altMarketsToAdd, otherMarketsToAdd);
    }

    function _becomeG3(uint altRate_, address[] memory altMarketsToAdd, address[] memory otherMarketsToAdd) public {
        require(msg.sender == comptrollerImplementation, "only brains can become itself");

        for (uint i = 0; i < altMarketsToAdd.length; i++) {
            _addMarketInternal(address(altMarketsToAdd[i]));
        }
        ...
        _setAltRate(altRate_);
        _addAltMarkets(altMarketsToAdd);
    }
