...
async function getAltMarkets(world: World, comptroller: Comptroller): Promise<ListV> {
  let mkts = await comptroller.methods.getAltMarkets().call();
...
export function comptrollerFetchers() {
  return [
    ...
    new Fetcher<{comptroller: Comptroller}, ListV>(`
      ...
      * "GetAltMarkets" - Returns an array of the currently enabled Alt markets. To use the auto-gen array getter altMarkets(uint), use AltMarkets
      * E.g. "Comptroller GetAltMarkets"
      `,
      "GetAltMarkets",
      [new Arg("comptroller", getComptroller, {implicit: true})],
      async(world, {comptroller}) => await getAltMarkets(world, comptroller)
     ),

    new Fetcher<{comptroller: Comptroller}, NumberV>(`
      #### AltRate

      * "AltRate" - Returns the current alt rate.
      * E.g. "Comptroller AltRate"
      `,
      "AltRate",
      [new Arg("comptroller", getComptroller, {implicit: true})],
      async(world, {comptroller}) => new NumberV(await comptroller.methods.altRate().call())
    ),
    ...
    new Fetcher<{comptroller: Comptroller, signature: StringV, callArgs: StringV[]}, NumberV>(`
        ...
        * "CallNum signature:<String> ...callArgs<CoreValue>" - Simple direct call method
          * E.g. "Comptroller CallNum \"altSpeeds(address)\" (Address Coburn)"
      `,
      "CallNum",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
    ...
    new Fetcher<{comptroller: Comptroller, AToken: AToken, key: StringV}, NumberV>(`
        #### AltSupplyState(address)

        * "Comptroller AltBorrowState cZRX "index"
      `,
      "AltSupplyState",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken, key}) => {
        const result = await comptroller.methods.altSupplyState(AToken._address).call();
        return new NumberV(result[key.val]);
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken, key: StringV}, NumberV>(`
        #### AltBorrowState(address)
      ...
      async (world, {comptroller, AToken, key}) => {
        const result = await comptroller.methods.altBorrowState(AToken._address).call();
        return new NumberV(result[key.val]);
      }
    ),
    new Fetcher<{comptroller: Comptroller, account: AddressV, key: StringV}, NumberV>(`
      ...
      async (world, {comptroller,account}) => {
        const result = await comptroller.methods.altAccrued(account.val).call();
        return new NumberV(result);
      }
    ),
    new Fetcher<{comptroller: Comptroller, account: AddressV, key: StringV}, NumberV>(`
        #### AltReceivable(address)

        * "Comptroller AltReceivable Coburn
      `,
      "AltReceivable",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller,account}) => {
        const result = await comptroller.methods.altReceivable(account.val).call();
        return new NumberV(result);
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken, account: AddressV}, NumberV>(`
        ...
        * "Comptroller AltSupplierIndex cZRX Coburn
      `,
      "AltSupplierIndex",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken, account}) => {
        return new NumberV(await comptroller.methods.altSupplierIndex(AToken._address, account.val).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken, account: AddressV}, NumberV>(`
        #### AltBorrowerIndex

        * "Comptroller AltBorrowerIndex cZRX Coburn
      `,
      "AltBorrowerIndex",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken, account}) => {
        return new NumberV(await comptroller.methods.altBorrowerIndex(AToken._address, account.val).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken}, NumberV>(`
        #### AltSpeed

        * "Comptroller AltSpeed cZRX
      `,
      "AltSpeed",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken}) => {
        return new NumberV(await comptroller.methods.altSpeeds(AToken._address).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken}, NumberV>(`
        #### AltSupplySpeed

        * "Comptroller AltSupplySpeed cZRX
      `,
      "AltSupplySpeed",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken}) => {
        return new NumberV(await comptroller.methods.altSupplySpeeds(AToken._address).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller, AToken: AToken}, NumberV>(`
        #### AltBorrowSpeed

        * "Comptroller AltBorrowSpeed cZRX
      `,
      "AltBorrowSpeed",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      async (world, {comptroller, AToken}) => {
        return new NumberV(await comptroller.methods.altBorrowSpeeds(AToken._address).call());
      }
    ),
    new Fetcher<{comptroller: Comptroller}, AddressV>(`
        #### BorrowCapGuardian
