...
describe('GovernorBravo#queue/1', () => {
  ...
  describe("overlapping actions", () => {
    it("reverts on queueing overlapping actions in same proposal", async () => {
      ...
      const alt = await deploy('Alt', [root]);
      const gov = await deploy('GovernorBravoImmutable', [timelock._address, alt._address, root, 17280, 1, "100000000000000000000000"]);
      await send(gov, '_initiate');
      const txAdmin = await send(timelock, 'harnessSetAdmin', [gov._address]);

      await enfranchise(alt, a1, 3e6);
      await mineBlock();

      const targets = [alt._address, alt._address];
      const values = ["0", "0"];
    ...
    it("reverts on queueing overlapping actions in different proposals, works if waiting", async () => {
      ...
      await enfranchise(alt, a1, 3e6);
      await enfranchise(alt, a2, 3e6);
      await mineBlock();

      const targets = [alt._address];
      const values = ["0"];
