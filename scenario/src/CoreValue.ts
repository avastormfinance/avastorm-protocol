...
import { getAltValue, altFetchers } from './Value/AltValue';
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let isAltKey = '0x' + toBN(newKey).add(toBN(3)).toString(16);
          let isAltStr = await world.web3.eth.getStorageAt(addr.val, isAltKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isAltStr, 1))
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ given: Value; expected: Value }, BoolV>(
    `
      ...
      * "Equal given:<Value> expected:<Value>" - Returns true if given values are equal
        ...
        * E.g. "Equal (AToken cZRX TotalSupply) (Exactly 55)"
        * E.g. "Equal (AToken cZRX Comptroller) (Comptroller Address)"
    `,
    'Equal',
  ...
  new Fetcher<{ res: Value }, Value>(
    ...
    `,
    'Alt',
    [new Arg('res', getAltValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: altFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Governor
...
let contractFetchers = [
  ...
  { contract: "AvastormLens", implicit: false },
