...
describe('AltScenario', () => {
  let root, accounts;
  let alt;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    alt = await deploy('AltScenario', [root]);
  });
    ...
    [
      ...
      it(`with ${checkpoints} checkpoints, has ${expectedReads} reads`, async () => {
        ...
        while (remaining > 0) {
          ...
          await alt.methods.generateCheckpoints(amt, offset).send({from: root, gas: 200000000});
          remaining -= amt;
          offset += amt;
        }

        let result = await alt.methods.getPriorVotes(root, 1).send();
