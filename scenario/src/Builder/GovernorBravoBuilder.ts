...
export async function buildGovernor(
  ...
  const fetchers = [
    new Fetcher<
      { name: StringV, timelock: AddressV, alt: AddressV, admin: AddressV, implementation: AddressV, votingPeriod: NumberV, votingDelay: NumberV, proposalThreshold: NumberV},
      GovernorBravoData
    >(
      `
      #### GovernorBravoDelegator

      * "GovernorBravo Deploy BravoDelegator name:<String> timelock:<Address> alt:<Address> admin:<Address> implementation<address> votingPeriod:<Number> votingDelay:<Number>" - Deploys Avastorm Governor Bravo with a given parameters
        * E.g. "GovernorBravo Deploy BravoDelegator GovernorBravo (Address Timelock) (Address Alt) Admin (Address impl) 17280 1"
    `,
      "BravoDelegator",
      [
        new Arg("name", getStringV),
        new Arg("timelock", getAddressV),
        new Arg("alt", getAddressV),
      ...
      async (world, { name, timelock, alt, admin, implementation, votingPeriod, votingDelay, proposalThreshold }) => {
        return {
          invokation: await GovernorBravoDelegator.deploy<GovernorBravo>(
            world,
            from,
            [timelock.val, alt.val, admin.val, implementation.val, votingPeriod.encode(), votingDelay.encode(), proposalThreshold.encode()]
          ),
          name: name.val,
    ...
    new Fetcher<
      { name: StringV, timelock: AddressV, alt: AddressV, admin: AddressV, votingPeriod: NumberV, votingDelay: NumberV, proposalThreshold: NumberV },
      GovernorBravoData
    >(
      `
      #### GovernorBravoImmutable

      * "GovernorBravoImmut Deploy BravoImmutable name:<String> timelock:<Address> alt:<Address> admin:<Address> votingPeriod:<Number> votingDelay:<Number>" - Deploys Avastorm Governor Bravo Immutable with a given parameters
        * E.g. "GovernorBravo Deploy BravoImmutable GovernorBravo (Address Timelock) (Address Alt) Admin 17280 1"
    `,
      "BravoImmutable",
      [
        new Arg("name", getStringV),
      ...
      async (world, { name, timelock, alt, admin, votingPeriod, votingDelay, proposalThreshold }) => {
        return {
          invokation: await GovernorBravoImmutable.deploy<GovernorBravo>(
            world,
            from,
            [timelock.val, alt.val, admin.val, votingPeriod.encode(), votingDelay.encode(), proposalThreshold.encode()]
          ),
          name: name.val,
    ...
    >(
      ...
      * "Governor Deploy BravoDelegate name:<String>" - Deploys Avastorm Governor Bravo Delegate
    ...
    >(
      ...
      * "Governor Deploy BravoDelegateHarness name:<String>" - Deploys Avastorm Governor Bravo Delegate Harness
